...
<body class="p-4 sm:p-8 min-h-screen flex items-center justify-center">
    ...
    <div class="w-full max-w-2xl bg-white shadow-2xl rounded-xl p-6 sm:p-10 border-t-8 border-indigo-500">
        ...
        <header class="mb-8 text-center">
            ...
            <p class="text-gray-600 leading-relaxed text-sm sm:text-base">
                同學好～針對國內畢業旅行，想聽聽同學願望，**不記名投票**。盡量可以滿足大家許可時間和想去的地點。歡迎攜家帶眷共襄盛舉。
            </p>
        ...
        <form id="travelForm" class="space-y-6">
            ...
            <div class="bg-gray-50 p-5 rounded-lg border border-gray-200">
                ...
                <input type="text" id="location" name="地點" required
                       placeholder="例如：花蓮太魯閣、墾丁、澎湖..."
            ...
            <div class="bg-gray-50 p-5 rounded-lg border border-gray-200">
                ...
                <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-6">
                    <label class="inline-flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="天數" value="兩天一夜" required
                               class="form-radio h-5 w-5 text-indigo-600 border-gray-300 focus:ring-indigo-500">
                        <span class="ml-3 text-base">兩天一夜 (2D1N)</span>
                    </label>
                    <label class="inline-flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="天數" value="三天兩夜"
                               class="form-radio h-5 w-5 text-indigo-600 border-gray-300 focus:ring-indigo-500">
                        <span class="ml-3 text-base">三天兩夜 (3D2N)</span>
                    </label>
                </div>
            </div>

            <!-- Question 3: Time Options -->
            <div class="bg-gray-50 p-5 rounded-lg border border-gray-200">
                <p class="block text-lg font-bold text-gray-800 mb-3">
                    3. 方便的旅遊時間選項？ (請選擇最合適的一項)
                </p>
                <div class="space-y-3">
                    <!-- Radio options for Time -->
                    <label class="flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="時間" value="5月第一週 (勞動節)" required
                               class="time-option form-radio h-5 w-5 text-indigo-600 focus:ring-indigo-500">
                        <span class="ml-3 text-base">5月第一週 (勞動節)</span>
                    </label>
                    <label class="flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="時間" value="5月第二週"
                               class="time-option form-radio h-5 w-5 text-indigo-600 focus:ring-indigo-500">
                        <span class="ml-3 text-base">5月第二週</span>
                    </label>
                    <label class="flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="時間" value="5月第三週"
                               class="time-option form-radio h-5 w-5 text-indigo-600 focus:ring-indigo-500">
                        <span class="ml-3 text-base">5月第三週</span>
                    </label>
                    <label class="flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="時間" value="5月第四週"
                               class="time-option form-radio h-5 w-5 text-indigo-600 focus:ring-indigo-500">
                        <span class="ml-3 text-base">5月第四週</span>
                    </label>
                    <label class="flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="時間" value="時間都可以"
                               class="time-option form-radio h-5 w-5 text-indigo-600 focus:ring-indigo-500">
                        <span class="ml-3 text-base">時間都可以</span>
                    </label>
                    <label class="flex items-center text-gray-700 cursor-pointer">
                        <input type="radio" name="時間" value="其他時間" id="otherTimeOption"
                               class="time-option form-radio h-5 w-5 text-indigo-600 focus:ring-indigo-500">
                        <span class="ml-3 text-base">其他時間</span>
                    </label>
                </div>
            </div>

            <!-- Question 4: Other Time Details (Conditional) -->
            <div id="otherTimeDetail" class="bg-yellow-50 p-5 rounded-lg border border-yellow-200 hidden transition-all duration-300">
                ...
                <input type="text" id="otherTimeText" name="其他時間說明"
                       placeholder="請具體說明您的時間，例如：6月第二週末"
            ...
            <button type="submit" id="submitButton"
                    ...
                    disabled>
                <span id="buttonText">送出問卷 (匿名)</span>
    ...
    <script>
        ...
        function setLoading(isLoading) {
            ...
            if (isLoading) {
                buttonText.textContent = '資料送出中...';
                loadingSpinner.classList.remove('hidden');
            } else {
                buttonText.textContent = '送出問卷 (匿名)';
        ...
        form.addEventListener('submit', async (e) => {
            ...
            for (let [key, value] of formData.entries()) {
                // Ensure the "其他時間說明" only gets sent if "其他時間" was selected
                if (key === '其他時間說明' && data['時間'] !== '其他時間') {
                    continue; 
                }
                data[key] = value.trim();
            }
            ...
            data['填寫時間'] = new Date().toLocaleString('zh-TW', { 
            ...
            try {
                ...
                // Note: The 'redirect' mode is necessary for Google Apps Script to work correctly
                ...
                // Since we use 'no-cors', we cannot inspect the response, but if fetch completes, we assume success.
                // A better approach would be to use fetch and handle the JSON response, but 'no-cors' is the easiest setup for new users.
                
                showMessage('success', '✅ 感謝您的填寫！問卷資料已成功送出。');
                form.reset();
                ...
                showMessage('error', '❌ 資料送出失敗，請檢查網路連線或稍後再試。');
            } finally {
                setLoading(false);
            }
        });
